"""Python controller for the Audio (AudioMass) Harvester."""
...
class AudioHarvester:
    """Manages JS harvester injection and telemetry parsing for Audio engines."""

    def __init__(self, engine_type: SandboxType) -> None:
        """Initialize the audio harvester."""
    ...
    def get_injection_js(self) -> str:
        ...
        if not self._js_cache:
            ...
            if self.engine_type == SandboxType.AUDIOMASS:
                harvester_path = self.static_dir / "audiomass_harvester.js"
            else:
                harvester_path = self.static_dir / "audiomass_harvester.js"
    ...
    def parse_telemetry(self, telemetry: SandboxTelemetry) -> object | None: # architectural: allowed-object (Audio State)
        """Map raw sandbox telemetry to specific Audio contracts."""
